...
if(Np > 0)
    ...
    [fig_p, ax_p] = getFig('$\varepsilon$', 'p',...
                      ['$p(\varepsilon) | p_0 = ' num2str(round(p0, prec)) '$'],...
                      'log');
    [fig_points, ax_points] = getFig('x', 'y');

    main_proc(a, b, x, p0, e_min, Np, prec, ax_p, ax_points, lines);
else
    N_arr = 1:15;
    N_N = length(N_arr);
    e_arr = zeros(N_N, 1);
    ...
    getFig('N', '$\varepsilon$', ['$p_0 = ' num2str(p0) '$']);
    plot(N_arr, e_arr, 'DisplayName', 'data');
    ...
    plot(N_arr, e_arr, 'x', 'DisplayName', 'data');
    plot(N_arr, exp(log_fit(1)*log(N_arr) + log_fit(2)), '-', 'DisplayName', 'line fit')    
end
